...
namespace WebAppApi.Repository
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<PageList<Message>> GetMessagesForUser(MessageParams messageParams)
        {
            ...
            {
                case "Inbox":
                    messages = messages.Where(u => u.RecipientId == messageParams.UserId && u.ReciptientDeleted==false);
                    break;
                case "Outbox":
                    messages = messages.Where(u => u.SenderId == messageParams.UserId && u.SenderDeleted == false);
                    break;
                default:
                    messages = messages.Where(u => u.RecipientId == messageParams.UserId && u.ReciptientDeleted == false && u.IsRead == false);
                    break;
            }
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientId)
        {
            var messages = await _context.Messages
                .Where(m => m.RecipientId == userId && m.ReciptientDeleted == false && m.SenderId == recipientId
